...
![ac_suggestions](ac_suggestions.png)
...
There are two search handlers defined for each map view in vb_ta (where xxxx is replaced by
...
1. **xxxxAcgrouped** handler returns the number of matching documents grouped (not faceted, more
   about group-faceting vs faceting [here](https://cwiki.apache.org/confluence/display/solr/Result+Grouping)) by category. This allows us to provide the user with estimates on
