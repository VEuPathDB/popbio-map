...
L.CustomColorFunction = L.PiecewiseFunction.extend({
    options: {interpolate: true}, initialize: function (d, c, a, j) {
        var h = c - d;
        var b = h / (a.length - 1);
        var f = [];
        var k;
        ...
        for (var e = 0; e < a.length; ++e) {
            var g = Math.min(e + 1, a.length - 1);
            k = this.options.interpolate ? new L.RGBColorBlendFunction(d + b * e, d + b * g, a[e], a[g]) : new L.RGBColorBlendFunction(d + b * e, d + b * g, a[e], a[e]);
            f.push(k)
        }
        L.PiecewiseFunction.prototype.initialize.call(this, f)
    }
});
...
L.LegendIcon = L.DivIcon.extend({
    initialize: function (c, f, k) {
        var a = document.createElement("div");
        var d = L.DomUtil.create("div", "legend", a);
        var e = L.DomUtil.create("div", "title", d);
        var b = L.DomUtil.create("div", "legend-box", d);
        var i = L.DomUtil.create("div", "legend-values", d);
        var h;
        var g = f.title || f.name;
        if (g) {
            e.innerHTML = g
        }
        for (var j in c) {
            h = c[j];
            L.DomUtil.create("div", "key", i).innerHTML = h.name || j;
            L.DomUtil.create("div", "value", i).innerHTML = h.value
        }
        L.StyleConverter.applySVGStyle(b, f);
        b.style.height = "5px";
        k.html = a.innerHTML;
        k.className = k.className || "legend-icon";
        L.DivIcon.prototype.initialize.call(this, k)
    }
});
...
L.HTMLUtils = {
    buildTable: function (c, f, b) {
        f = f || "table table-condensed table-striped table-bordered";
        var i = L.DomUtil.create("table", f);
        var e = L.DomUtil.create("thead", "", i);
        var d = L.DomUtil.create("tbody", "", i);
        e.innerHTML = "<tr><th>Name</th><th>Value</th></tr>";
        b = b || [];
        function j(m, o) {
        ...
        for (var h in c) {
            if (c.hasOwnProperty(h) && !j(b, h)) {
                var g = c[h];
                if (typeof g === "object") {
                    var a = document.createElement("div");
                    a.appendChild(L.HTMLUtils.buildTable(g, b));
                    g = a.innerHTML
                }
                d.innerHTML += "<tr><td>" + h + "</td><td>" + g + "</td></tr>"
            }
        }
        return i
    }
...
L.RegularPolygon = L.Polygon.extend({
    statics: {R: 6378.137, M_PER_KM: 1000}, initialize: function (b, a) {
        this._centerLatLng = b;
        L.Util.setOptions(this, a);
        L.Polygon.prototype.initialize.call(this, this._getLatLngs(), a)
    }, options: {fill: true, radius: 1000, numberOfSides: 4, rotation: 0, maxDegrees: 360}, getLatLng: function () {
        ...
        var b = d / Math.max(this.options.numberOfSides, 3);
        var f = d + this.options.rotation;
        ...
        return a
    }, _getPoint: function (b) {
        var g = function (j) {
            return j * L.LatLng.DEG_TO_RAD
        };
        var a = function (j) {
            return j * L.LatLng.RAD_TO_DEG
        };
        var i = g(b);
        var h = this.options.radius / L.RegularPolygon.M_PER_KM / L.RegularPolygon.R;
        var d = g(this._centerLatLng.lat);
        var f = g(this._centerLatLng.lng);
        var c = Math.asin(Math.sin(d) * Math.cos(h) + Math.cos(d) * Math.sin(h) * Math.cos(i));
        var e = f + Math.atan2(Math.sin(i) * Math.sin(h) * Math.cos(d), Math.cos(h) - Math.sin(d) * Math.sin(c));
        c = a(c);
        e = a(e);
        return new L.LatLng(c, e)
    }, toGeoJSON: function () {
        var b = {type: "Feature", geometry: {type: "Polygon", coordinates: [[], []]}, properties: this.options};
        for (var a = 0; a < this._latlngs.length; ++a) {
            var c = this._latlngs[a];
            b.coordinates[0].push([c[1], c[0]])
        }
        return b
    }
});
L.regularPolygon = function (b, a) {
    return new L.RegularPolygon(b, a)
};
...
L.DataLayer = L.LayerGroup.extend({
    ...
    _getLegendElement: function (y) {
        var q;
        var u;
        var s = document.createElement("i");
        var j = y.displayProperties;
        var c = y.layerOptions;
        var e = y.ignoreProperties;
        var a = y.displayTextFunction;
        var g = y.index;
        var m = y.numSegments;
        var p = y.segmentWidth;
        var l = y.minValue;
        var v = y.maxValue;
        L.StyleConverter.applySVGStyle(s, c);
        for (var f in j) {
            if (e.indexOf(f) === -1) {
                valueFunction = j[f];
                if (valueFunction && (valueFunction.getBounds || (j.minValue && j.maxValue))) {
                    var h = valueFunction.getBounds ? valueFunction.getBounds() : null;
                    var z = h ? h[0].x : j.minValue;
                    var w = h ? h[1].x : j.maxValue;
                    var b = new L.LinearFunction(new L.Point(0, z), new L.Point(m, w));
                    q = z;
                    u = w;
                    if (a) {
                        q = a(z);
                    ...
                    var d = (w - z) / m;
                    var k = b.evaluate(g);
                    var A = b.evaluate(g + 1);
                    var n = valueFunction.evaluate ? valueFunction.evaluate(k) : valueFunction(k);
                    var t = valueFunction.evaluate ? valueFunction.evaluate(A) : valueFunction(A);
                    L.StyleConverter.setCSSProperty(s, f, n);
                    if (f === "fillColor") {
                        if (y.gradient) {
                            s.style.cssText += "background-image:linear-gradient(left , " + n + " 0%, " + t + " 100%);background-image:-ms-linear-gradient(left , " + n + " 0%, " + t + " 100%);background-image:-moz-linear-gradient(left , " + n + " 0%, " + t + " 100%);background-image:-webkit-linear-gradient(left , " + n + " 0%, " + t + " 100%);"
                        } else {
                            s.style.cssText += "background-color:" + t + ";"
                        }
                    }
                    if (f === "color") {
                        s.style.cssText += "border-top-color:" + n + ";border-bottom-color:" + t + ";border-left-color:" + n + ";border-right-color:" + t + ";"
                    }
                    if (f === "weight") {
                        s.style.cssText += "border-top-width:" + n + ";border-bottom-width:" + t + ";border-left-width:" + n + ";border-right-width:" + t + ";"
                    }
                    var o = (d * g) + z;
                    var r = o + d;
                    if (a && valueFunction) {
    ...
    _getLegend: function (i) {
        i = i || this.options.legendOptions || {};
        var e = i.className;
        var r = document.createElement("div");
        var f = L.DomUtil.create("div", "legend", r);
        var u = i.numSegments || 10;
        var t = i.width || 100;
        var b = this.options.layerOptions || {};
        var p = b.weight || 0;
        var v = (t / u) - 2 * p;
        var w;
        var q = this.options.displayOptions || {};
        if (e) {
            L.DomUtil.addClass(f, e)
        }
        if (i.title) {
            L.DomUtil.create("legend", "", f).innerHTML = i.title
        }
        var m = function (z) {
            return z
        };
        for (var c in q) {
            var k = q[c];
            if (!k.excludeFromLegend) {
                var h = k.displayName || c;
                w = k.displayText;
                var a = w ? w : m;
                var l = k.styles;
                L.DomUtil.create("div", "legend-title", f).innerHTML = h;
                if (l) {
                    f.innerHTML += new L.CategoryLegend(l).generate()
                } else {
                    var o = L.DomUtil.create("div", "data-layer-legend");
                    var s = L.DomUtil.create("div", "min-value", o);
                    var y = L.DomUtil.create("div", "scale-bars", o);
                    var x = L.DomUtil.create("div", "max-value", o);
                    var g = ["displayName", "displayText", "minValue", "maxValue"];
                    for (var j = 0; j < u; ++j) {
                        var n = {
                            displayProperties: k,
                            layerOptions: b,
                        ...
                        var d = this._getLegendElement(n);
                        y.appendChild(d)
                    }
                    f.appendChild(o)
                }
...
L.Control.Legend = L.Control.extend({
    options: {position: "bottomright", autoAdd: true}, onAdd: function (d) {
        var c = "leaflet-control-legend", a = L.DomUtil.create("div", c);
        var b = this;
        if (this.options.autoAdd) {
            d.on("layeradd", function (g) {
                var f = g.layer;
                b.addLayer(f)
            });
            d.on("layerremove", function (g) {
                var f = g.layer;
                b.removeLayer(f)
            })
        }
        this.toggleSize = L.bind(this.toggleSize, this);
        L.DomEvent.addListener(a, "mouseover", this.toggleSize).addListener(a, "mouseout", this.toggleSize).addListener(a, "touchstart", this.toggleSize).addListener(a, "touchend", this.toggleSize).addListener(a, "click", L.DomEvent.stopPropagation).addListener(a, "click", L.DomEvent.preventDefault);
        return a
    }, clear: function () {
        ...
        var b = this;
        if (a.getLegend) {
            this.addLegend(c, a.getLegend());
            a.on("legendChanged", function () {
                b.redrawLayer(a)
            })
        }
    }, removeLayer: function (b) {
        var c = L.Util.stamp(b);
        if (b.getLegend) {
            var a = document.getElementById(c);
            a.parentNode.removeChild(a);
            b.off("legendChanged")
        }
    }, addLegend: function (d, b) {
        var a = this._container, c = document.getElementById(d);
        if (!c) {
            c = L.DomUtil.create("div", "", a);
            c.id = d
        }
        c.innerHTML = b
    }
});
L.control.legend = function (a) {
    return new L.Control.Legend(a)
};