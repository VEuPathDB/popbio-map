function loadSolr(e) {
    ...
    var d = [];
    var c = function (t) {
        $("#markersCount").html(t.response.numFound + " samples in current view");
        if (t.response.numFound === 0) {
            if (a) {
        ...
        switch (f) {
            case 1:
            case 2:
                p = t.stats.stats_fields.geo_coords_ll_0___tdouble.facets.geohash_1;
                s = t.stats.stats_fields.geo_coords_ll_1___tdouble.facets.geohash_1;
                r = t.facet_counts.facet_pivot["geohash_1,species_category"];
                break;
            case 3:
            case 4:
            case 5:
                p = t.stats.stats_fields.geo_coords_ll_0___tdouble.facets.geohash_2;
                s = t.stats.stats_fields.geo_coords_ll_1___tdouble.facets.geohash_2;
                r = t.facet_counts.facet_pivot["geohash_2,species_category"];
                break;
            case 6:
            case 7:
                p = t.stats.stats_fields.geo_coords_ll_0___tdouble.facets.geohash_3;
                s = t.stats.stats_fields.geo_coords_ll_1___tdouble.facets.geohash_3;
                r = t.facet_counts.facet_pivot["geohash_3,species_category"];
                break;
            case 8:
            case 9:
                p = t.stats.stats_fields.geo_coords_ll_0___tdouble.facets.geohash_4;
                s = t.stats.stats_fields.geo_coords_ll_1___tdouble.facets.geohash_4;
                r = t.facet_counts.facet_pivot["geohash_4,species_category"];
                break;
            case 10:
            case 11:
                p = t.stats.stats_fields.geo_coords_ll_0___tdouble.facets.geohash_5;
                s = t.stats.stats_fields.geo_coords_ll_1___tdouble.facets.geohash_5;
                r = t.facet_counts.facet_pivot["geohash_5,species_category"];
                break;
            default:
                p = t.stats.stats_fields.geo_coords_ll_0___tdouble.facets.geohash_6;
                s = t.stats.stats_fields.geo_coords_ll_1___tdouble.facets.geohash_6;
                r = t.facet_counts.facet_pivot["geohash_6,species_category"];
                break
        }
        smallClusters = [];
        var i = [];
        ...
        for (var n in p) {
            if (p.hasOwnProperty(n)) {
                ...
                r.forEach(function (x, w, y) {
                    i[x.value] = x.count;
                    var v = [];
                    var u = [];
                    if (x.pivot) {
                        x.pivot.forEach(function (A) {
                            var z = A.value, B = A.count;
                            v[z] = B;
                            u.push({
                                label: z.replace(/sensu lato/, "sl").replace(/chromosomal form/, "cf"),
                                value: B,
                                color: (palette[z] ? palette[z] : "#000000")
                            })
                        })
                    } else {
                        console.log("ERROR: Pivot for element " + x.value + "appears to be empty")
                    }
                    h[x.value] = v;
                    o[x.value] = u
                });
                ...
                j.term = n;
                j.count = p[n].count;
                j.latLng = [p[n].mean, s[n].mean];
                j.bounds = [[p[n].min, s[n].min], [p[n].max, s[n].max]];
                j.population = i[n];
                j.stats = h[n];
                j.fullstats = o[n];
                d.push(j)
        ...
        var q = {
            ...
            onEachRecord: function (v, u) {
                v.on("dblclick", function () {
                    map.fitBounds(u.bounds)
                });
                v.on("click", function () {
                    updatePieChart(u.population, u.fullstats)
                });
                v.on("mouseout", function () {
                })
            }
        };
        var k = new L.MarkerDataLayer(m, q);
        if (a) {
            assetLayerGroup.clearLayers();
...
function loadSmall(g, f) {
        ...
        options: {
            ...
            for (var r in this.stats) {
                if (this.stats.hasOwnProperty(r)) {
                    var t = this.stats[r] / this.population;
                    if (t > 0) {
                        k.beginPath();
                        k.moveTo(m, m);
    ...
    var a = "http://funcgen.vectorbase.org/popbio-map-preview/asolr/solr/vb_popbio/select?" + qryUrl + "&fq=bundle_name:Sample&fq=has_geodata:true&q.op=OR&fq=" + h + ":" + d + "&rows=10000000" + buildBbox(map.getBounds()) + "&fl=geo_coords,species_category&wt=json&json.nl=map&json.wrf=?&callback=?";
    map.spin(true);
    $.getJSON(a, b)
}
