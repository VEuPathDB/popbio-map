function loadSolr(i) {
    ...
    var e = [];
    var h = function (w) {
        $("#markersCount").html(w.response.numFound + " samples");
...
function loadSmall(h, e) {
    var f = new PruneClusterForLeaflet(100);
    ...
    var c = "http://funcgen.vectorbase.org/popbio-map-preview/asolr/solr/vb_popbio/select?q=bundle_name:Sample AND has_geodata:true" + qryUrl + "&fq=" + d + ":" + j + "&rows=10000000" + l + "&fl=geo_coords,species_category&wt=json&json.nl=map&json.wrf=?&callback=?";
    map.spin(true);
    $.getJSON(c, k)
}
