function loadSolr(e) {
    ...
    var d = [];
    var c = function (u) {
        $("#markersCount").html(u.response.numFound + " samples in current view");
...
function loadSmall(g, d) {
    ...
    var b = "http://funcgen.vectorbase.org/popbio-map-preview/asolr/solr/vb_popbio/select?" + qryUrl + "&fq=bundle_name:Sample&fq=has_geodata:true&fq=" + c + ":" + h + "&rows=10000000" + buildBbox(map.getBounds()) + "&fl=geo_coords,species_category&wt=json&json.nl=map&json.wrf=?&callback=?";
    map.spin(true);
    $.getJSON(b, j)
}
