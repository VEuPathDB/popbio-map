        ...
        x.done(function (T) {
            ...
            if (C > 50) {
                ...
                if (C >= 10 && C <= 50) {
                    ...
                    O.done(function (V) {
                        if (V.grouped.phenotype_value_type_s.matches > 0) {
                            H = V.grouped.phenotype_value_type_s.groups[0];
                            E = e / 2;
                            I = [];
                            K = 4 * (y - B) / e;
                            H.doclist.docs.forEach(function (X, W) {
                                I.push({
                                    x: undefined,
                                    y: X.phenotype_value_f,
                                    species: X.species_category[0],
                                    insecticide: X.insecticide_s,
                                    concentration: X.concentration_f,
                                    concentration_unit: X.concentration_unit_s,
                                    duration: X.duration_f,
                                    duration_unit: X.duration_unit_s
                                })
                            });
                            D = new Beeswarm(I, 0, K);
            ...
            $.when(G, R).done(function (ad, W) {
                var ae = W[0].response.numFound, ak = W[0].facets.pmin, ab = W[0].facets.pmax;
                var Y = k.append("g").attr("transform", "translate(" + (1 * (e + q) + h.left) + ",0)");
                var ac, Z, ah, ai = e / 2, aa, V;
                var ag, X, aj, af;
                if (ae > 50) {
                    if (W[0].facets.count > 0) {
                        ag = W[0].facets.denplot.buckets;
                        X = W[0].facets.pmean;
                        aj = W[0].facets.pperc;
                        addViolin(Y, ag, [j - h.bottom, h.top], e, Q, p, f, false);
                        addBoxPlot(Y, aj, X, [j - h.bottom, h.top], e, Q, 0.15, false)
                    }
                } else {
                    if (ae >= 10 && ae <= 50) {
                        if (ad[0].grouped.phenotype_value_type_s.matches > 0) {
                            Z = ad[0].grouped.phenotype_value_type_s.groups[0];
                            ai = e / 2;
                            ac = [];
                            aa = 4 * (ab - ak) / e;
                            Z.doclist.docs.forEach(function (am, al) {
                                ac.push({
                                    x: undefined,
                                    y: am.phenotype_value_f,
                                    species: am.species_category[0],
                                    insecticide: am.insecticide_s,
                                    concentration: am.concentration_f,
                                    concentration_unit: am.concentration_unit_s,
                                    duration: am.duration_f,
                                    duration_unit: am.duration_unit_s
                                })
                            });
                            ah = new Beeswarm(ac, 0, aa);
                            V = [(e - 8 * ah.maxPoints) / 2, (e - 8 * ah.maxPoints) / 2 + 8 * ah.maxPoints];
                            if (8 * ah.maxPoints > e) {
                                V = [0, e]
                            }
                            addBeeswarm(Y, ah, [j - h.bottom, h.top], V, Q, ah.domain, false)
                        }
                        if (W[0].facets.count > 0) {
                            X = W[0].facets.pmean;
                            aj = W[0].facets.pperc;
