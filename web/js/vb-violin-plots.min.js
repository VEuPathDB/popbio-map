function addViolin(l, m, p, n, o, q, i, c) {
    ...
    t.append("path").datum(m).attr("class", "area").attr("d", s).on("mouseover", function () {
        ...
        var A = C.length - 1;
        if (v === 0) {
            var y = "Values " + C[v].val.roundDecimals(4) + "-" + (C[v].val + h).roundDecimals(4);
            var z = C[v].count
        } else {
            if (v < A) {
                var y = "Values " + C[v - 1].val.roundDecimals(4) + "-" + (C[v - 1].val + h).roundDecimals(4);
                var z = C[v - 1].count
            } else {
                var y = "Values " + C[A - 1].val.roundDecimals(4) + "-" + (C[A - 1].val + h).roundDecimals(4);
                var z = C[A - 1].count
            }
        }
        var x = '<h3 style="background-color: #CCCCCC;"><font color="white">Count of phenotypes</font></h3><p>%BIN: %COUNT</p>'.replace("%BIN", y).replace("%COUNT", z);
        a.html(x).style("left", (d3.event.pageX + 5) + "px").style("top", (d3.event.pageY - 28) + "px")
    });
