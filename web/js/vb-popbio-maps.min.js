        ...
        i.on("click", function () {
            ...
            timer = setTimeout(function () {
                if (!prevent) {
                    ...
                    updateTable("#table-contents", buildBbox(n))
                }
                prevent = false
            }, delay)
        });
...
function updatePieChart(a, b) {
    if (b) {
        ...
        d3.select("#pie-chart-area svg").attr("width", 380).attr("height", 500).style({width: "380", height: "500"});
...
function updateTable(h, b) {
    var g = h + "-header";
    $(g).empty();
    if ($("#view-mode").val() === "smpl") {
        var d = "http://funcgen.vectorbase.org/popbio-map-preview/asolr/solr/vb_popbio/smplTable?&" + qryUrl + b + "&sort=id asc&json.wrf=?&callback=?"
    } else {
        var d = "http://funcgen.vectorbase.org/popbio-map-preview/asolr/solr/vb_popbio/irTable?&" + qryUrl + b + "&sort=id asc&json.wrf=?&callback=?"
    }
    var c = d + "&cursorMark=*", f = "*", e;
    PaneSpin("marker-table", "start");
    var a = this;
    $(h).empty();
    $("#marker-table").off("scroll");
    $.getJSON(c).done(function (j) {
        if (j.response.numFound && j.response.numFound > 0) {
            var k = j.response.docs;
            e = j.nextCursorMark;
            c = d + "&cursorMark=" + e;
            tableHtml(h, k)
        }
        ...
        $("#marker-table").scroll(function () {
            if (($(this)[0].scrollHeight - $(this).scrollTop() === $(this).outerHeight(true)) && (f !== e)) {
                PaneSpin("marker-table", "start");
                $.getJSON(c).done(function (l) {
                    if (l.response.numFound && l.response.numFound > 0) {
                        var m = l.response.docs;
                        f = e;
                        e = l.nextCursorMark;
                        c = d + "&cursorMark=" + e;
                        tableHtml(h, m)
...
function generatePalette(p) {
    ...
    if ($("#view-mode").val() === "ir") {
        d += '<div class="data-layer-legend" style="border: 0">';
        d += "<p>Resistance</p>";
        d += '<div class="min-value" style="border: 0">Low</div>';
        d += '<div class="scale-bars">';
        $.each(L.ColorBrewer.Diverging.RdYlBu[7].reverse(), function (q, r) {
            d += '<i style="margin: 0; border-radius: 0; border: 0; color: ' + r + "; width: 10px; background-color: " + r + ' ;"></i>'
        });
        d += '</div><div class="max-value" style="border: 0">High</div></div><p style="font-size: smaller; word-wrap: break-word; width: 200px; margin-top: 20px">Values have been rescaled globally and only give a relative indication of resistance/susceptibility</p>'
    }
...
function resetPlots() {
    var b = '<h3>Sample summary data</h3><div id="pie-chart-header" style="text-align: center; margin-top: 30px"><i class="fa fa-pie-chart" style="color: #2c699e; font-size: 12em"></i><h3>click a marker</h3></div><div id="pie-chart-area"><svg></svg></div>';
    var c = '<div style="text-align: center; margin-top: 30px"><i class="fa fa-area-chart" style="color: #2c699e; font-size: 12em"></i><h3>click a marker</h3></div>';
    var a = '<div style="text-align: center; margin-top: 30px"><i class="fa fa-th-list" style="color: #2c699e; font-size: 12em"></i><h3>click a marker</h3></div>';
    $("#graphs").html(b);
