function initializeMap() {
    map = L.map("map", {center: [23.079, 3.515], zoom: 3, zoomControl: false, worldCopyJump: true});
    map.spin(true);
...
function initializeSearch() {
            ...
            datasets: [{
                ...
                templates: {
                    empty: function () {
                        var c;
                        if ($("#world-search").val() === "1") {
                            c = "No suggestions found. Hit Enter to perform a free text search instead."
                        } else {
                            c = "No suggestions found. Try enabling world search or hit enter to perform a free text search instead."
                        }
    ...
    $("#SelectView").find("li a").click(function () {
        ...
        $.getJSON(c, function (e) {
            legend.populateLegend(e, "species_category")
        });
        a.initialize(true);
        b.initialize(true)
    })
...
function loadSolr(l) {
    var i = l.clear;
    var f = l.zoomLevel;
    var c = geohashLevel(f, "geohash");
    var j = map.getBounds();
    var g = j.getSouth(), d = j.getNorth(), a = j.getEast(), e = j.getWest();
    if (urlParams.grid === "true") {
        addGeohashes(map, g, e, d, a, c.slice(-1))
    }
    if (f > 11) {
        loadSmall(1, f);
        return
    ...
    var h = [];
    var k = function (C) {
        $("#markersCount").html(C.response.numFound + " samples in current view");
        if (C.response.numFound === 0) {
            if (i) {
        ...
        var y, B, A, m;
        var u = C.stats.stats_fields;
        var v = C.facet_counts;
        var o = $("#view-mode").val();
        switch (f) {
            case 1:
            case 2:
                y = u.geo_coords_ll_0_coordinate.facets.geohash_1;
                B = u.geo_coords_ll_1_coordinate.facets.geohash_1;
                m = (o === "ir") ? u.phenotype_rescaled_value_f.facets.geohash_1 : null;
                A = v.facet_pivot["geohash_1,species_category"];
                break;
            case 3:
            case 4:
            case 5:
                y = u.geo_coords_ll_0_coordinate.facets.geohash_2;
                B = u.geo_coords_ll_1_coordinate.facets.geohash_2;
                m = ((o === "ir") ? u.phenotype_rescaled_value_f.facets.geohash_2 : null);
                A = v.facet_pivot["geohash_2,species_category"];
                break;
            case 6:
            case 7:
                y = u.geo_coords_ll_0_coordinate.facets.geohash_3;
                B = u.geo_coords_ll_1_coordinate.facets.geohash_3;
                m = ((o === "ir") ? u.phenotype_rescaled_value_f.facets.geohash_3 : null);
                A = v.facet_pivot["geohash_3,species_category"];
                break;
            case 8:
            case 9:
                y = u.geo_coords_ll_0_coordinate.facets.geohash_4;
                B = u.geo_coords_ll_1_coordinate.facets.geohash_4;
                m = (o === "ir") ? u.phenotype_rescaled_value_f.facets.geohash_4 : null;
                A = v.facet_pivot["geohash_4,species_category"];
                break;
            case 10:
            case 11:
                y = u.geo_coords_ll_0_coordinate.facets.geohash_5;
                B = u.geo_coords_ll_1_coordinate.facets.geohash_5;
                m = (o === "ir") ? u.phenotype_rescaled_value_f.facets.geohash_5 : null;
                A = v.facet_pivot["geohash_5,species_category"];
                break;
            default:
                y = u.geo_coords_ll_0_coordinate.facets.geohash_6;
                B = u.geo_coords_ll_1_coordinate.facets.geohash_6;
                m = (o === "ir") ? u.phenotype_rescaled_value_f.facets.geohash_6 : null;
                A = v.facet_pivot["geohash_6,species_category"];
                break
        }
        smallClusters = [];
        var p = [];
        ...
        for (var w in y) {
            if (y.hasOwnProperty(w)) {
                ...
                A.forEach(function (F, G) {
                    ...
                    var D = [];
                    if (F.pivot) {
                        F.pivot.forEach(function (I) {
                            var H = I.value, J = I.count;
                            E[H] = J;
                            D.push({
                                label: H.replace(/sensu lato/, "sl").replace(/chromosomal form/, "cf"),
                                value: J,
                                color: (palette[H] ? palette[H] : "#000000")
                            })
                        })
                    } else {
                        console.log("ERROR: Pivot for element " + F.value + "appears to be empty")
                    }
                    n[F.value] = E;
                    x[F.value] = D
                });
                ...
                q.term = w;
                q.count = y[w].count;
                q.latLng = [y[w].mean, B[w].mean];
                q.bounds = [[y[w].min, B[w].min], [y[w].max, B[w].max]];
                q.population = p[w];
                q.trafficlight = (o === "ir") ? m[w].mean : -1;
                q.stats = n[w];
                q.fullstats = x[w];
                h.push(q)
            }
        ...
        var z = {
            ...
            onEachRecord: function (E, D) {
                ...
                E.on("click", function () {
                    ...
                    timer = setTimeout(function () {
                        if (!prevent) {
                            ...
                            updatePieChart(D.population, D.fullstats);
                            var G = L.latLngBounds(D.bounds);
                            createBeeViolinPlot("#swarm-chart-area", buildBbox(G));
                            updateTable("#table-contents", buildBbox(G))
                        }
                    }, delay);
                    prevent = false
                });
                E.on("mouseover", function () {
                    moveTopMarker(E);
                    var F = L.latLngBounds(D.bounds);
                    if (rectHighlight !== null) {
                        map.removeLayer(rectHighlight)
                    }
                    rectHighlight = L.rectangle(F, {color: "grey", weight: 1, fill: true, clickable: false}).addTo(map)
                });
        ...
        var r = new L.MarkerDataLayer(t, z);
        if (i) {
            assetLayerGroup.clearLayers();
            assetLayerGroup.addLayer(r)
        } else {
            assetLayerGroup.addLayer(r)
        }
    ...
    var b = solrPopbioUrl + $("#view-mode").val() + "Geoclust?" + qryUrl + buildBbox(map.getBounds()) + "&stats.facet=" + c + "&facet.pivot=" + c + ",species_category&json.wrf=?&callback=?";
    map.spin(true);
    $.getJSON(b, k).fail(function () {
        console.log("Ahhh");
        map.spin(false)
    })
}
function loadSmall(g, f) {
    var e = new PruneClusterForLeaflet(120);
        ...
        i.on("click", function () {
            ...
            timer = setTimeout(function () {
                ...
                if (!prevent) {
                    ...
                    updatePieChart(1, n);
                    var o = "&fq=id:" + l.id;
                    createBeeViolinPlot("#swarm-chart-area", o);
                    updateTable("#table-contents", o)
                }
                prevent = false
            }, delay)
        });
    ...
    e.BuildLeafletCluster = function (k, j) {
        ...
        i.on("click", function () {
            ...
            timer = setTimeout(function () {
                if (!prevent) {
                    ...
                    updatePieChart(k.population, n);
                    var o = e.Cluster.FindMarkersInArea(k.bounds);
                    var m = e.Cluster.ComputeBounds(o);
                    if (m) {
                        var r = new L.LatLngBounds(new L.LatLng(m.minLat, m.maxLng), new L.LatLng(m.maxLat, m.minLng))
                    }
                    createBeeViolinPlot("#swarm-chart-area", buildBbox(r));
                    updateTable("#table-contents", buildBbox(r))
                }
            }, delay);
            prevent = false
        });
        i.on("mouseover", function () {
            ...
            if (l) {
                var m = new L.LatLngBounds(new L.LatLng(l.minLat, l.maxLng), new L.LatLng(l.maxLat, l.minLng))
            }
            if (rectHighlight !== null) {
                map.removeLayer(rectHighlight)
            }
            rectHighlight = L.rectangle(m, {color: "grey", weight: 1, fill: true, clickable: false}).addTo(map)
        });
...
function updateTable(e, b, i) {
    ...
    if ($("#view-mode").val() === "smpl") {
        var a = solrPopbioUrl + "smplTable?&" + qryUrl + b + "&sort=id asc&json.wrf=?&callback=?"
    } else {
        var a = solrPopbioUrl + "irTable?&" + qryUrl + b + "&sort=id asc&json.wrf=?&callback=?"
    }
...
function moveTopMarker(a) {
    $(a._icon).addClass("top-marker")
}
function removeTopMarker(a) {
    $(a._icon).removeClass("top-marker")
}
...
function resetPlots() {
    var b, c, a;
    if (firstClick) {
        b = '<h3>Summary view for selected samples</h3><div id="pie-chart-header" style="text-align: center; margin-top: 30px"><i class="fa fa-pie-chart" style="color: #2c699e; font-size: 12em"></i><h1>Go on!</h1><h4>click a marker on the map</h4><h4>to plot some real data</h4> </div><div id="pie-chart-area"><svg></svg></div>';
        c = '<div style="text-align: center; margin-top: 30px"><i class="fa fa-area-chart" style="color: #2c699e; font-size: 12em"></i><h1>Go on!</h1><h4>click a marker on the map</h4><h4>to plot some real data</h4> </div>';
        a = '<div style="text-align: center; margin-top: 30px"><i class="fa fa-table" style="color: #2c699e; font-size: 12em"></i><h1>Go on!</h1><h4>click a marker on the map</h4><h4>to see some real data</h4> </div>'
    } else {
        b = '<h3>Summary view for selected samples</h3><div id="pie-chart-header" style="text-align: center; margin-top: 30px"><i class="fa fa-pie-chart" style="color: #2c699e; font-size: 12em"></i><h4>click a marker on the map</h4></div><div id="pie-chart-area"><svg></svg></div>';
        c = '<div style="text-align: center; margin-top: 30px"><i class="fa fa-area-chart" style="color: #2c699e; font-size: 12em"></i><h4>click a marker on the map</h4></div>';
        a = '<div style="text-align: center; margin-top: 30px"><i class="fa fa-table" style="color: #2c699e; font-size: 12em"></i><h4>click a marker on the map</h4></div>'
    }
    $("#graphs").html(b);
        ...
        function f(o, k, n) {
            ...
            if (k.indexOf(".") !== -1) {
                ...
                if (l !== "") {
                    ...
                    console.warn('parseParams :: empty property in key "' + k + '"')
                }
            } else {
                if (k.indexOf("[") !== -1) {
                    var m = k.split("[");
