...
function updateTable(e, b) {
    var d = e + "-header";
    $(d).empty();
    if ($("#view-mode").val() === "smpl") {
        var c = "http://funcgen.vectorbase.org/popbio-map-preview/asolr/solr/vb_popbio/smplTable?&" + qryUrl + b + "&json.wrf=?&callback=?"
    } else {
        var c = "http://funcgen.vectorbase.org/popbio-map-preview/asolr/solr/vb_popbio/irTable?&" + qryUrl + b + "&json.wrf=?&callback=?"
    }
    PaneSpin("marker-table", "start");
    var a = this;
    $(e).empty();
    $.getJSON(c).done(function (f) {
        if (f.response.numFound && f.response.numFound > 0) {
            var g = f.response.docs;
            tableHtml(e, g)
        }
    }).fail(function () {
...
function tableHtml(b, a) {
    a.forEach(function (h) {
        ...
        var k = {
            ...
            species: h.species_category[0],
            bgColor: palette[h.species_category[0]],
            textColor: getContrastYIQ(palette[h.species_category[0]]),
            collectionDate: f,
            projects: h.projects,
            collectionProtocols: h.collection_protocols
        };
        var j = $.templates("#rowTemplate");
        var g = j.render(k);
        $(b).append(g)
    });
    PaneSpin("marker-table", "stop")
