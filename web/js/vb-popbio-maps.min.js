function initializeMap() {
    ...
    var f = L.control.sidebar("sidebar").addTo(map);
...
function loadSolr(e) {
    ...
    var d = [];
    var c = function (v) {
        $("#markersCount").html(v.response.numFound + " samples in current view");
        if (v.response.numFound === 0) {
            if (a) {
        ...
        var u;
        var t;
        var n = v.stats.stats_fields;
        var o = v.facet_counts;
        switch (f) {
            case 1:
            case 2:
                r = n.geo_coords_ll_0_coordinate.facets.geohash_1;
                u = n.geo_coords_ll_1_coordinate.facets.geohash_1;
                t = o.facet_pivot["geohash_1,species_category"];
                break;
            case 3:
            case 4:
            case 5:
                r = n.geo_coords_ll_0_coordinate.facets.geohash_2;
                u = n.geo_coords_ll_1_coordinate.facets.geohash_2;
                t = o.facet_pivot["geohash_2,species_category"];
                break;
            case 6:
            case 7:
                r = n.geo_coords_ll_0_coordinate.facets.geohash_3;
                u = n.geo_coords_ll_1_coordinate.facets.geohash_3;
                t = o.facet_pivot["geohash_3,species_category"];
                break;
            case 8:
            case 9:
                r = n.geo_coords_ll_0_coordinate.facets.geohash_4;
                u = n.geo_coords_ll_1_coordinate.facets.geohash_4;
                t = o.facet_pivot["geohash_4,species_category"];
                break;
            case 10:
            case 11:
                r = n.geo_coords_ll_0_coordinate.facets.geohash_5;
                u = n.geo_coords_ll_1_coordinate.facets.geohash_5;
                t = o.facet_pivot["geohash_5,species_category"];
                break;
            default:
                r = n.geo_coords_ll_0_coordinate.facets.geohash_6;
                u = n.geo_coords_ll_1_coordinate.facets.geohash_6;
                t = o.facet_pivot["geohash_6,species_category"];
                break
        }
        smallClusters = [];
        var i = [];
        ...
        for (var p in r) {
            if (r.hasOwnProperty(p)) {
                var l = r[p].count;
                if (l < 2) {
                    smallClusters.push(p);
                    continue
                }
                t.forEach(function (y, z) {
                    i[y.value] = y.count;
                    var x = [];
                    var w = [];
                    if (y.pivot) {
                        y.pivot.forEach(function (B) {
                            var A = B.value, C = B.count;
                            x[A] = C;
                            w.push({
                                label: A.replace(/sensu lato/, "sl").replace(/chromosomal form/, "cf"),
                                value: C,
                                color: (palette[A] ? palette[A] : "#000000")
                            })
                        })
                    } else {
                        console.log("ERROR: Pivot for element " + y.value + "appears to be empty")
                    }
                    h[y.value] = x;
                    q[y.value] = w
                });
                ...
                j.term = p;
                j.count = r[p].count;
                j.latLng = [r[p].mean, u[p].mean];
                j.bounds = [[r[p].min, u[p].min], [r[p].max, u[p].max]];
                j.population = i[p];
                j.stats = h[p];
                j.fullstats = q[p];
                d.push(j)
        ...
        var s = {
            ...
            setIcon: function (w) {
                ...
                return new L.Icon.Canvas({
                    iconSize: new L.Point(x, x),
                    className: "marker-cluster",
                    population: w.population,
                    stats: w.stats
                })
            },
            onEachRecord: function (x, w) {
                x.on("dblclick", function () {
                    map.fitBounds(w.bounds)
                });
                x.on("click", function () {
                    updatePieChart(w.population, w.fullstats);
                    var y = L.latLngBounds(w.bounds);
                    createBeeViolinPlot("#swarm-chart-area", buildBbox(y))
                });
                x.on("mouseover", function (y) {
                });
                x.on("mouseout", function () {
                })
            }
        };
        var k = new L.MarkerDataLayer(m, s);
        if (a) {
            assetLayerGroup.clearLayers();
            assetLayerGroup.addLayer(k)
        } else {
            assetLayerGroup.addLayer(k)
        }
...
function loadSmall(g, f) {
    ...
    e.PrepareLeafletMarker = function (i, l, k) {
        ...
        i.on("click", function () {
            ...
            updatePieChart(1, m);
            var n = L.latLngBounds(i._latlng, i._latlng);
            createBeeViolinPlot("#swarm-chart-area", buildBbox(n))
        });
        if (l.icon) {
    ...
    L.Icon.MarkerCluster = L.Icon.extend({
        options: {
            ...
            var m = Math.PI * 2;
            var i = Math.PI * 1.5;
            var n = this.options.iconSize.x, l = n / 2, k = n / 2.5;
            for (var q in this.stats) {
                if (this.stats.hasOwnProperty(q)) {
                    var r = this.stats[q] / this.population;
                    if (r > 0) {
                        j.beginPath();
                        j.moveTo(l, l);
                        if (palette.hasOwnProperty(q)) {
                            j.fillStyle = palette[q]
                        } else {
                            j.fillStyle = palette.others
                        }
                        var p = i, o = i + r * m;
                        if (o < p) {
                            p = i
            ...
            j.beginPath();
            j.fillStyle = "white";
            j.arc(l, l, k, 0, Math.PI * 2);
            j.fill();
            j.closePath();
            j.fillStyle = "#555";
            j.textAlign = "center";
            j.textBaseline = "middle";
            j.font = "bold 12px sans-serif";
            j.fillText(this.population, l, l, n)
        }
    });
    ...
    var b = function (i) {
        ...
        for (var l in n) {
            if (n.hasOwnProperty(l)) {
                ...
                var j = new PruneCluster.Marker(m[0], m[1]);
                if (n[l].hasOwnProperty("species_category")) {
                    var k = n[l].species_category[0];
                    j.category = n[l].species_category[0]
                } else {
...
function updatePieChart(a, b) {
    if (b) {
        ...
        d3.select("#pie-chart-area svg").style({width: "380", height: "500"});
...
function generatePalette(p) {
    var k = p.facet_counts.facet_pivot["geohash_2,species_category"];
    var h = [];
    for (var c in k) {
    ...
    var j = sortHashByValue(h);
    palette = buildPalette(j, legendSpecies, 1);
    var d = "";
    var l = 1;
    for (var b in palette) {
        if (l > legendSpecies - 1) {
            d += '<i style="background:' + palette.others + '"></i> Others<br />';
            $("#legend").html(d);
            break
        }
        var a = b.replace(/^(\w{2})\S+\s(\w+)/, "$1. $2");
        d += '<i style="background:' + palette[b] + '" title="' + b + '"></i> ' + (b ? "<em>" + a + "</em><br>" : "+");
        l++
    }
    legend.onAdd = function (q) {
